...
import { UserExistException } from "src/exception/UserExistException";
...
export class AuthService {
  ...
  async login(loginDto: ILoginDto): Promise<OLoginDto> {
    ...
    this.userTokenManager.addToken({
      refreshToken: resultDto.refreshToken,
      user,
      rootToken: resultDto.refreshToken,
    });
  ...
  async refreshToken(refreshTokenDto: IRefreshTokenDto): Promise<OLoginDto> {
    ...
    const userToken = await this.userTokenManager.getToken(
      username,
      refreshToken
    );
    if (!userToken) {
    ...
    if (userToken.isUsed) {
      const unUsedTokens = await this.userTokenManager.listUnUsedToken(
        refreshToken
      );
  ...
  async register(registerDto: IRegisterDto) {
    const existUser = await this.userManager.findByUsername(
